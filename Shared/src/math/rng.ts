 ...
 *
 * The simulation-determinism contract (Docs/02 §4): no `Math.random`, no host
 * trig for state — every random draw in Shared code flows through an Rng
 * handed down from the world seed, and subsystems isolate their streams with
 * `fork(label)` so adding a draw in one system never perturbs another.
 */
