...
async function makeTransport(): Promise<{ transport: Transport; mode: "online" | "solo" }> {
  ...
  worker.postMessage({ __cfg: { bots: numParam("bots"), enemies: numParam("enemies"), cdscale: numParam("cdscale"), seed: params.get("seed") ?? undefined, reachIndex: numParam("reach") } });
